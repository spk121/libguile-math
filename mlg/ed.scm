...
(define (main args)
  (let* ((option-spec '((prompt (single-char #\p) (value #t) (predicate string?))
    ...
    (while #t
      ...
      (cond
       ;; FIXME: I'm going down to ops to parse messages that should
       ;; be handled at this level, like HELP and HELP_MODE.  Really
       ;; they should be parsed at this level, and then flowed down to
       ;; ops if they aren't HELP or HELP_MODE.
       ((eqv? status 'help)
        (format-error "~a~%" errmsg))
       ((eqv? status 'help-mode)
        (set! garrulous (not garrulous))
        (when garrulous
          (format-error "~a~%" errmsg))))
      (when (and (or (not status)
                     (and (integer? status) (< status 0)))
                 garrulous)
        (format-error "~a~%" errmsg))
      (if verbose
          (%dump-cbuffer cbuf))
      ...
      ;; This is where input is parsed and commands are run.
      (let ((line-cur (if (zero? (get-line-count cbuf))
        ...
        (let ((addr-range (addr-get-range port
          ...
          ;; If status is ERR, update the last error.
          ;; FIXME?:. Now I have errors strings in three places: address
          ;; ops, and here.
          (when (eqv? status ERR)
            (seterrmsg (op-get-dispatch-error)))
