...
will be merged into a single string using (format #f param1 param2 ...)"
  ...
  (let ((fields-alist (acons "PRIORITY" (log-level->priority log-level) '())))
    ...
    (log-structured-array log-level fields-alist)))

;; 1906: g_log_structured_array
(define (log-structured-array log-level fields-alist)
  ;;(pk 'log-structured-array 'log-level log-level 'fields-alist fields-alist)
   ...
   (else
    (let ((fields-alist
	   (append
		...
		`(("DOMAIN" . ,log-domain))
		'()))))
      (log-structured-array (logand log-level (lognot-uint16 LOG_FLAG_FATAL))
			    fields-alist)))))
